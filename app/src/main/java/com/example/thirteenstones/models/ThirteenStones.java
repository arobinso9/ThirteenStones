...
public class ThirteenStones {
    ...
    private boolean mFirstPlayerTurn, mWinnerIsLastPlayerToPick;
    ...
    private void updateGameWinStatisticsIfGameHasJustEnded() {
        if (isGameOver())
            mArrayPlayerWinCount[getWinningPlayerNumber() - 1]++;
    }
    ...
    public int getNumberOfWinsForPlayer(int playerNumber) {
        if (playerNumber < 1 || playerNumber > sPLAYER_COUNT)
            throw new IllegalArgumentException("Player number must be between 1 and " + sPLAYER_COUNT + ".");
